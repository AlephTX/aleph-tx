...
impl RiskGate {
    ...
    pub fn check_order(
        &self,
        order: &OrderRequest,
        orderbook: Option<&LocalOrderbook>,
    ) -> Result<(), RiskError> {
        // 1. Trading paused?
        if self.paused {
        ...
        if new_position.abs() > self.config.max_position_usd {
            return Err(RiskError::PositionOverflow(
                current_position,
                position_delta,
        ...
        if let Some(ob) = orderbook
            && let (Some(best_bid), Some(best_ask)) = (ob.best_bid(), ob.best_ask())
        {
            let mid = (best_bid.price + best_ask.price) / Decimal::from(2);
            let spread = best_ask.price - best_bid.price;

            // Extremely simple spoof detection - check if spread is artificially wide or thin
            // compared to a simplistic 1 tick or 50 tick logic
            if spread > Decimal::from(100) || spread < Decimal::from(1) {
                return Err(RiskError::SpreadAnomaly(spread, mid, Decimal::ZERO)); // Using 0 for threshold as it's a fixed check
            }
        }
